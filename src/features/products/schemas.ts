...
export function makeProductFormSchema(skuAutoGenerated: boolean) {
  ...
  .object({
    ...
    // A company category id, or '' for none. No shape rule, for the same reason as
    // initialVendorId: the picker only offers ids the server just sent.
...
/**
 * The update half of the payload: `toProductPayload`, plus the one field an update has to say
 * differently.
 *
 * An absent `categoryId` means "leave it alone" on update, so choosing "No category" has to be
 * sent as `clearCategory: true`. It is only sent when the product actually had a category —
 * `previousCategoryId` is the saved product's — so saving an uncategorised product unchanged
 * sends nothing new.
 */
export function toProductUpdatePayload(
  ...
  if (!values.categoryId && previousCategoryId != null) payload.clearCategory = true
  return payload
}
