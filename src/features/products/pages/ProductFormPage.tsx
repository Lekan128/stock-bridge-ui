...
import { renderSku } from '@/features/products/skuPattern'
...
export function ProductFormPage() {
   ...
   * degrade-toward-freedom choice `stockUnitLocked` makes on an unknown state below: a false
   * "off" briefly shows an editable, required SKU field, which is exactly today's behaviour and
   * costs nothing; a false "on" would lock a field that should have been free to type into.
   */
  ...
  const {
    ...
    // validation time, not once at mount, so this stays correct the moment `skuAutoGenerated`
    // resolves from `false` (its loading default) to whatever the fetched setting actually is.
    resolver: zodResolver(makeProductFormSchema(skuAutoGenerated)),
  ...
  /**
   * The create-form's disabled, pre-filled SKU field. Fetched ONCE per form load — see
   * `previewSkuDisplay` below for why re-rendering it as the name changes needs no further
   * network call. `nextSequence` is the one part of the preview the client can't compute itself
   * (the counter lives in `product_sku_settings`, server-side only); everything else the pattern
   * can produce is pure string substitution the client already has what it needs for.
   */
  const [skuNextSequence, setSkuNextSequence] = useState<number | null>(null)
  const skuPreviewFetched = useRef(false)
  useEffect(() => {
    if (isEdit || !skuAutoGenerated || skuPreviewFetched.current) return
    skuPreviewFetched.current = true
    productsApi
      .previewSku()
      .then((preview) => setSkuNextSequence(preview.nextSequence))
      .catch(() => {
        // Best-effort. The field stays blank until save, which still succeeds — the server
        // generates the real value itself regardless of what this preview showed.
      })
  }, [isEdit, skuAutoGenerated])

  /**
   * Re-rendered locally, live, from the pattern (`GET /api/products/sku-settings`, already
   * fetched above) and the once-fetched `nextSequence` — no network call as the name changes.
   * This is what fixes the create form showing a blank `{NAME:N}` part until save: the previous
   * version only asked the server for a preview once, before anything was typed, and never
   * updated it. Purely a display value — `save` below never reads it, and the server ignores
   * whatever the (hidden, unused-when-auto-generated) `sku` form field carries regardless.
   */
  const previewSkuDisplay =
    skuNextSequence != null ? renderSku(skuSettings?.pattern ?? '', skuNextSequence, watch('name')) : null
  ...
  return (
    <div className="mx-auto flex max-w-2xl flex-col gap-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-col gap-5">
        ...
        {/* Locked whenever the tenant has auto-generation on — see ProductSkuSettingsPage for the
            toggle. Create mode shows a live, client-rendered preview (`previewSkuDisplay`); edit
            mode shows the product's real sku. An edit-mode holder of PRODUCT_SKU_OVERRIDE (V23)
            can unlock it for this one product; everyone else just sees the generated value.
            Mirrors the locked stock-unit field's shape below: a hidden input keeps
            react-hook-form's value intact while the box shows it as read-only text with the
            reason on screen. */}
        {skuAutoGenerated && !skuUnlocked ? (
          <div>
            <div className="mb-1.5 flex flex-wrap items-center justify-between gap-x-2 gap-y-1">
              {/* Same reasoning the locked stock-unit label above uses: no htmlFor when there is
                  no focusable control for it to point at. */}
              <label className="block text-sm font-medium text-neutral-700">SKU</label>
              {isEdit && canOverrideSku && (
                <button
                  type="button"
            ...
            <div className="flex items-center gap-2 rounded-md border border-neutral-200 bg-neutral-50 px-3 py-2">
              ...
              {/* Edit mode shows the product's real, already-saved sku (registered above via
                  ...
                  round trip. `save` below never reads either of these; the server decides the
                  real value at commit time regardless. */}
              <span className="text-sm font-medium text-neutral-700">
                {(isEdit ? watch('sku') : previewSkuDisplay) || 'Generating…'}
              </span>
            </div>
            <p className="mt-1.5 text-xs text-neutral-500">
              {isEdit ? 'Generated automatically for this product.' : 'Generated automatically — confirmed once you save.'}
            </p>
          </div>
        ) : (
